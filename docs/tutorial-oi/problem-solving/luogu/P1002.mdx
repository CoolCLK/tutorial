...
显而易见，像这类求路径数量的题目都涉及到了 [动态规划](../../programming-ideas/dynamic-programming.mdx)。
那么我们可以从题目得到 $B(n,m), \text{马}(p,q)$，设 $\text{卒}(i, j)$。
...
int main() {
    ...
    std::vector<std::vector<l_long>> dp(n + 1, std::vector<l_long>(m + 1, 0));
    std::vector<std::vector<bool>> controlled(n + 1, std::vector<bool>(m + 1, false));
