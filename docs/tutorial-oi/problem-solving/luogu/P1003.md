...
public:
    ...
    rectangle(int x, int y, int w, int h) {
        this -> position = std::pair(x, y);
        this -> size = std::pair(w, h);
    }

    bool is_in_bounding_box(std::pair<int, int> other) {
...
int main() {
    int n;
    scanf("%d", &n);
    std::vector<rectangle*> carpets(0, nullptr);
    repeat(n) {
        int a, b, g, k;
    ...
    for (int i = carpets.size() - 1; i >= 0; i--) { // 优先取最上面的，因而从后往前遍历
        ...
        if (carpet->is_in_bounding_box(std::pair(x, y))) {
            printf("%d\n", i + 1);
            return 0;
        }
    }
