...
原始题目与提交代码请前往 [P1055 [NOIP 2008 普及组] ISBN 号码 - 洛谷](https://www.luogu.com.cn/problem/P1055)。
...
int main() {
  int isbn[10];
  char c;
  scanf("%1d-%1d%1d%1d-%1d%1d%1d%1d%1d-%c", &isbn[0], &isbn[1], &isbn[2], &isbn[3], &isbn[4], &isbn[5], &isbn[6], &isbn[7], &isbn[8], &c);
  isbn[9] = c == 'X' ? 10 : c - '0';
  int r = 0;
  for (int i = 0; i < 9; i++) {
    r += isbn[i] * (i + 1);
  ...
  if (isbn[9] == r) {
    ...
    printf("%d-%d%d%d-%d%d%d%d%d-%c\n", isbn[0], isbn[1], isbn[2], isbn[3], isbn[4], isbn[5], isbn[6], isbn[7], isbn[8], r == 10 ? 'X' : char(r + '0'));
  }
  return 0;
}
```
