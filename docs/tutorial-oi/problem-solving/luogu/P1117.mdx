...
public:
    string_part(std::string* str, size_t index, size_t length) {
        this->str = str;
        this->index = index;
        this->length = length;
    }
...
u_int find_aabb(std::string str) {
    u_int ans = 0;
    std::vector<string_part> strs;
...
int main() {
    scanf("%hu", &T);
    std::vector<u_int> answers;
    repeat(T) {
        std::string S;
		std::cin >> S;
        answers.emplace_back(find_aabb(S));
    }
