...
u_short n, m;
u_short min_time = 65535;
u_short max_time = 0;
std::vector<std::pair<short, short>> directions = {std::pair(0, 1), std::pair(0, -1), std::pair(1, 0), std::pair(-1, 0)};

int main() {
    scanf("%hu\n", &n, &m);
    std::vector<std::vector<bool>> borders(n, std::vector<bool>(n, false));
    repeat (m) {
        u_short x, y;
        scanf("%hu\n", &x, &y);
...
```cpp showLineNumbers
void dfs(u_short x, u_short y, u_short time, std::vector<std::vector<bool>>& visited) {
    //if (x == n - 1 && y == n - 1) {
    if (x == 0 && y == n - 1) {
