...
std::vector<u_int> ans(0);
std::vector<std::vector<u_int>> tree;
std::vector<u_int> parent, depth;
u_int N, M, S;

void process_tree(u_int __top, u_int __parent, u_int __depth) {
    parent[__top - 1] = __parent; // 记录父节点
    depth[__top - 1] = __depth; // 记录深度
    for (auto& __node : tree[__top - 1]) {
        if (__node == __parent) continue; // 避免回环
        process_tree(__node, __top, __depth + 1); // 递归处理子节点
    }
}

int main() {
    scanf("%u %u %u", &N, &M, &S);
    tree = std::vector<std::vector<u_int>>(N, std::vector<u_int>(0));
    parent = std::vector<u_int>(N, 0);
    depth = std::vector<u_int>(N, 0);
    repeat(N - 1) {
        u_int x, y;
        scanf("%u %u", &x, &y);
        tree[x - 1].push_back(y);
    ...
    process_tree(S, S, 1); // 根节点的父节点为自身，深度为 1
