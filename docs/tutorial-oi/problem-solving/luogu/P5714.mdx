...
原始题目与提交代码请前往 [P5714 【深基3.例7】肥胖问题 - 洛谷](https://www.luogu.com.cn/problem/P5714)。
...
int main() {
  ...
  double h;
  scanf("%hu %lf", &m, &h);
  double bmi = static_cast<double>(m) / h / h;
  if (bmi >= 24) {
    printf("%.6g\n", bmi);
  }
  printf("%s\n", bmi < 18.5 ? "Underweight" : (bmi >= 18.5 && bmi < 24 ? "Normal" : "Overweight"));
  return 0;
}
```
